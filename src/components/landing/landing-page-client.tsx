...
function LandingPageContent({ galleryImagePaths }: { galleryImagePaths: string[] }) {
    return (
        <>
            <HeroSection />
            <Partners />
            ...
            <CtaSection />
            <Testimonials />
            <GalleryPreview imagePaths={galleryImagePaths} />
            <SocialCta />
        </>
    );
}

export function LandingPageClient({ galleryImagePaths }: { galleryImagePaths: string[] }) {
  const { loading } = useAuth();

  if (loading) {
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-background gap-4">
        <Image src="/load.png" alt="VisionHub Logo" width={200} height={50} />
        <p className="text-muted-foreground animate-pulse">Unleashing Creativity...</p>
      </div>
    );
  }

  return <LandingPageContent galleryImagePaths={galleryImagePaths} />;
}
